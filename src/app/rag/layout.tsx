...
export default function RAGLayout({ children }: RAGLayoutProps) {
  ...
  const [activeView, setActiveView] = useState<"left" | "main" | "right">(
    "main"
  );
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Link href="/">
              <h1 className="text-xl font-bold text-slate-900 hover:text-primary transition-colors cursor-pointer">
                LegalRAG
              </h1>
            </Link>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          className={`${
            activeView === "left" ? "w-full" : "w-0"
          } xl:w-[20%] transition-all duration-300 overflow-hidden bg-white border-r border-slate-200 flex flex-col`}
        >
          {/* Sidebar Header */}
          <div className="p-4">
            <div className="flex items-center justify-between">
        ...
        <div
          className={`${
            activeView === "main" ? "w-full" : "w-0"
          } xl:w-[60%] transition-all duration-300 overflow-hidden`}
        >
          {children}
        </div>
        ...
        <div
          className={`${
            activeView === "right" ? "w-full" : "w-0"
          } xl:w-[20%] transition-all duration-300 overflow-hidden`}
        >
          <PdfToolsSidebar isOpen={true} onToggle={toggleRightSidebar} />
        </div>
      </div>
    </div>
  );
